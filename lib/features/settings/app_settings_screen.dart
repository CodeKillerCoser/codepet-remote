...
class _AppSettingsScreenState extends State<AppSettingsScreen> {
  ...
  Future<void> _export() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('日志压缩包已生成')),
      );
    } catch (error) {
  ...
  Future<void> _share() async {
    ...
    try {
      ...
      widget.logger.warning(
        'Diagnostics share sheet failed',
        error: error,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('无法打开系统分享：$error')),
      );
    }
  }
        ...
        body: ListView(
          ...
          children: [
            const ListTile(
              ...
              subtitle: Text(
                '设备凭据保存在 Android Keystore 支持的安全存储中；会话和事件游标不落盘。',
              ),
            ),
            const Divider(),
            ListTile(
